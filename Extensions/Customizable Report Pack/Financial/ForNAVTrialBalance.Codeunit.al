Codeunit 6188677 "ForNAV Trial Balance"
{
    ...
    begin
        ...
        if Args."Variance in Changes" or Args."% Variance in Changes" then
            TrialBalance."Variance in Changes" := TrialBalance."Net Change Actual" - TrialBalance."Net Change Actual Last Year";
        if Args."% Variance in Changes" and (TrialBalance."Net Change Actual Last Year" <> 0) then
            TrialBalance."% Variance in Changes" := TrialBalance."Variance in Changes" / TrialBalance."Net Change Actual Last Year" * 100;
        if Args."Variance in Balances" or Args."% Variance in Balances" then
            TrialBalance."Variance in Balances" := TrialBalance."Balance at Date Actual" - TrialBalance."Balance at Date Act. Last Year";
        if Args."% Variance in Balances" and (TrialBalance."Balance at Date Act. Last Year" <> 0) then
            TrialBalance."% Variance in Balances" := TrialBalance."Variance in Balances" / TrialBalance."Balance at Date Act. Last Year" * 100;
