table 6188510 "ForNAV Local Printer"
{
    ...
    fields
    {
        ...
        field(9; "Paper Source Kind"; Option)
        {
            ...
            trigger OnValidate()
            begin
                if "Paper Source Kind" <> "Paper Source Kind"::Custom then
        ...
        field(10; "Custom Paper Source Name"; Text[30])
        {
            ...
            trigger OnValidate()
            begin
                if "Custom Paper Source Name" <> '' then
                    "Paper Source Kind" := "Paper Source Kind"::Custom;
            end;
        }
        field(20; Unit; Option)
        {
            Caption = 'Unit';
            DataClassification = SystemMetadata;
    ...
    keys { key(PK; "Cloud Printer Name") { Clustered = true; } }
    ...
    trigger OnRename()
    begin
        if xRec."Cloud Printer Name" = 'Default' then
            Error('The default printer cannot be renamed.');
    end;

    trigger OnModify()
    begin
        if (xRec."Cloud Printer Name" = 'Default') and
            (Rec."Local Printer Name" <> 'DEFAULT')
        then
            Error('The local printer must be DEFAULT for the Default printer.');
        SetValues;
    end;

    trigger OnDelete()
    begin
        if "Cloud Printer Name" = 'Default' then
            Error('The default printer cannot be deleted.');
    end;
    ...
    begin
        ...
        if "Scale Mode" <> "Scale Mode"::CustomScale then
            Scale := 100;
    end;
    ...
    begin
        ...
        if (Duplex <> Duplex::Default) and (Duplex <> Duplex::Simplex) then
            payload.Add('duplex', true);
        if Description <> '' then
            payload.Add('description', Description);
        if Color then
            payload.Add('color', Color);
        if Copies <> 1 then
            payload.Add('defaultcopies', Copies);
        if "Paper Source Kind" = "Paper Source Kind"::Custom then
            paperTray.Add('papersourcekind', 257)
        else
            paperTray.Add('papersourcekind', "Paper Source Kind");
        if Paperkind = Paperkind::Default then
            paperTray.Add('paperkind', 9) // A4
        else
        ...
        if Landscape = LandScape::Landscape then
            paperTray.Add('landscape', true);
