...
{
    procedure CreateBuffer(var ReportLayout: Record "ForNAV Report Layout")
    var
    ...
    begin
        if ReportLayout.GetFilter(ReportLayout."Report ID") <> '' then begin
            ...
            if Evaluate(ID, ReportLayout.GetFilter(ReportLayout."Report ID")) then begin
                ...
                if RepLaySel.FindFirst then
                    ReportLayout.Activated := false;
                ReportLayout.Modify;
            end;
        end;

        CustRepLayout.SetFilter("Company Name", '%1|%2', '', CompanyName);
        if CustRepLayout.FindSet then
            repeat
                if IsForNAVReport.IsForNAVReport(CustRepLayout."Report ID") then begin
                    ReportLayout.Init;
                    ReportLayout."Entry No." += 1;
                    ...
                    if RepLaySel.IsEmpty then
                        RepLaySel.SetFilter("Company Name", '%1', '');
                    if RepLaySel.FindFirst then
                        ReportLayout.Activated := ReportLayout."Custom Report Layout Code" = RepLaySel."Custom Report Layout Code";
                    ReportLayout.Modify;
                end;
    ...
    procedure CloneDefaultLayout(ReportID: Integer)
    var
        AllObjWithCaption: Record AllObjWithCaption;
        CustRepLayout: Record "Custom Report Layout";
    ...
    procedure ClearAllForNAVLayouts()
    var
        CustRepLayout: Record "Custom Report Layout";
    ...
    procedure GetReportsWithSameDataSet(ReportID: Integer; var ReportIDs: list of [Integer])
    var
    ...
    begin
        ...
        repeat
            ...
            if ReportDataItem.FindFirst then
                DataSetIsMatch := true
            else
                DataSetIsMatch := false;
