table 6188484 "ForNAV Layout Import Worksheet"
{
    ...
    fields
    {
        field(1; "Report ID"; Integer) { DataClassification = SystemMetadata; Editable = false; }
        field(2; "Report Name"; Text[50]) { DataClassification = SystemMetadata; Editable = false; }
        field(3; Description; Text[250]) { DataClassification = SystemMetadata; Editable = false; }
        field(5; "Custom Layout Code"; Code[50]) { DataClassification = SystemMetadata; Editable = false; }
        ...
        field(21; Blob; Blob) { DataClassification = SystemMetadata; Caption = 'Blob', Comment = 'DO NOT TRANSLATE'; }
    }

    keys { key(PK; "Report ID", "Custom Layout Code") { Clustered = true; } }
    ...
    begin
        ...
        CalcFields("Report Exists", "Existing Report Name");
        if not "Report Exists" then
            exit;

        "Import Action" := "Import Action"::Create;
        if not CustRepLayout.Get("Custom Layout Code") then
            exit;


        if CustRepLayout."Report ID" = "Report ID" then
            "Import Action" := "Import Action"::Replace;

    end;

    procedure ReplaceLayout()
    var
        CustRepLayout: Record "Custom Report Layout";
    ...
    begin
        ...
        if RepLayoutSel.Get("Report ID", CompanyName) then
            RepLayoutSel.Delete;
        CreateLayout;
    end;
    ...
    begin
        ...
        CustRepLayout.Type := CustRepLayout.Type::Rdlc;
