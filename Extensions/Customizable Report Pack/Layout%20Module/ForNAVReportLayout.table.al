table 6188483 "ForNAV Report Layout"
{
    ...
    fields
    {
        field(1; "Report ID"; Integer) { Editable = false; DataClassification = SystemMetadata; }
        field(2; "Name"; Text[250]) { Editable = false; DataClassification = SystemMetadata; }
        field(3; "Custom Report Layout Code"; Code[20])
        {
            DataClassification = SystemMetadata;
            trigger OnValidate()
            var
                CustRepLayoutOld: Record "Custom Report Layout";
        ...
        field(10; Activated; Boolean) { DataClassification = SystemMetadata; }
        field(50; "Built-in"; Boolean) { Editable = false; DataClassification = SystemMetadata; }
        field(99; "Entry No."; Integer) { Editable = false; DataClassification = SystemMetadata; }
    }
    ...
    var
        ReportLayoutExport: Codeunit "ForNAV Report Layout Export";
    begin
    ...
    begin
        if "Built-in" then begin
            CustRepLayout.InitBuiltInLayout("Report ID", CustRepLayout.Type::Rdlc.AsInteger());
        end else begin
            CustRepLayout.Get("Custom Report Layout Code");
    ...
    var
        ReportObject: Record "ForNAV Report Object" temporary;
    ...
    begin
        ...
        repeat
            ReportObject.FindSet;
            repeat
                ReportDataItem.SetRange("Report ID", ReportObject.ID);
                ReportDataItem.SetRange(Name, CurrReportDataItem.Name);
                if ReportDataItem.IsEmpty then
                    ReportObject.Delete;
            until ReportObject.Next = 0;
        until CurrReportDataItem.Next = 0;

        Commit;
        if Page.RunModal(Page::"ForNAV Report Objects", ReportObject) <> Action::LookupOK then
            exit;

        if "Built-in" then begin
            CustRepLayout.InitBuiltInLayout(ReportObject.ID, CustRepLayout.Type::Rdlc.AsInteger());
        end else begin
            CustRepLayout.Get("Custom Report Layout Code");
            CustRepLayout.CalcFields(Layout);
