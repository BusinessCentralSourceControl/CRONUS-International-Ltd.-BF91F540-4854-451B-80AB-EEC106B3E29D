Page 6188475 "ForNAV Reports"
{
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                field(Name; Rec.Name) { ApplicationArea = All; Style = Strong; StyleExpr = Indent = 1; }
            ...
            action(Run)
            {
                ...
                begin
                    if Rec.ID = 0 then
                        exit;
                    Report.Run(Rec.ID);
                    if archive.GetArchiveSetup(Rec.ID, Rec.Archived) then
                        Rec.Modify();
                end;
            }
            action(LocalPrint)
    ...
    procedure GetPrinterSelected(): Text
    var
    ...
    begin
        if Rec.ID = 0 then
            ...
            if printerSelection.Get(UserId, Rec.ID) then
                Printer := printerSelection."Printer Name"
            else
                if printerSelection.Get('', Rec.ID) then
                    Printer := printerSelection."Printer Name"
                else
                    if printerSelection.Get(UserId, 0) then
                        Printer := printerSelection."Printer Name"
                    else
    ...
    begin
        case Rec.Layout of
            Rec.Layout::Created:
                LayoutStyle := 'ambiguous';
            Rec.Layout::Activated:
                LayoutStyle := 'strongaccent';
            rec.Layout::"Defined Externally":
                LayoutStyle := 'attention';
            else
                LayoutStyle := '';
        end;
        Rec.Printer := GetPrinterSelected();
    ...
    var
        GetForNAVReports: Codeunit "ForNAV Get ForNAV Reports";
        ...
        if Rec.FindSet then
            repeat
                ...
                if not Rec.get(Rec.Category, 0) then begin
                    ...
                    if printerSelection.Get(UserId, Rec.ID) then
                        Rec.Printer := printerSelection."Printer Name"
                    else
                        if printerSelection.Get('', Rec.ID) then
                            Rec.Printer := printerSelection."Printer Name"
                        else
    ...
    local procedure EnableOrShowArchive()
    var
    ...
    begin
        if (Rec.ID <> 0) then begin
            if archiveSetup.GetArchiveSetup(Rec.ID) then begin
                ...
                if (Confirm(enableArchiving, true)) then
                    archiveSetup.SetArchiveSetup(Rec.ID, true);
            end;
        end;
    end;
