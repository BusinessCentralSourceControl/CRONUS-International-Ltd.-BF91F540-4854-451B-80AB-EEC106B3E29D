page 6189180 "ForNAV Language Setup"
{
    ...
    begin
        if CurrPage.Translations.Page.GetSelectedLanguage = '' then
            Rec.SetRange("Language Code")
        else
            Rec.SetRange("Language Code", CurrPage.Translations.Page.GetSelectedLanguage);
    end;

    local procedure TestObjectFields()
    begin
        if Rec."Report ID" + Rec."Table No." + Rec."Field No." = 0 then
            Error('Please put in either a Report ID, Table No. or Field No.');
    end;

    local procedure SetObjectFilter()
    begin
        if ShowObjectDetails then
            Rec.SetRange("Is Object Translation", true)
        else
