table 6189180 "ForNAV Language Setup"
{
    ...
    fields
    {
        ...
        field(12; "Translate From"; Text[100]) { Caption = 'Translate From'; DataClassification = SystemMetadata; NotBlank = true; }
        field(15; "Translate To"; Text[250]) { Caption = 'Translate To'; DataClassification = SystemMetadata; }
        field(18; "Windows Language ID"; Integer) { DataClassification = SystemMetadata; Editable = false; }
        field(20; "Is Object Translation"; Boolean) { Caption = 'Is Object Translation', Comment = 'DO NOT TRANSLATE'; DataClassification = SystemMetadata; Editable = false; }
        field(99; Blob; Blob) { DataClassification = SystemMetadata; }
    }

    keys
    {
        key(PK; "Language Code", "Report ID", "Table No.", "Field No.", "Translate From") { Clustered = true; }
    }
    ...
    procedure ImportFromCSV(InStr: InStream)
    var
        CSVBuffer: Record "CSV Buffer" temporary;
        RecRef: RecordRef;
    ...
    begin
        ...
        if CSVBuffer.GetNumberOfColumns <> 6 then
            Error('The CSV file should contain exactly 6 columns');
        ...
        repeat
            ...
            for i := 1 to 6 do begin
                ...
                case FldRef[i].Number of
                    FieldNo("Report ID"), FieldNo("Table No."), FieldNo("Field No."):
                        begin
                            no := Variant;
                            if no <> 0 then
            ...
            if not RecRef.Insert then
                if not Confirm(TranslationExist, true, RecRef.CurrentKey) then
                    Error('')
                else
                    RecRef.Modify;
    ...
    procedure GetDefaultTranslations()
    var
        CSVBuffer: Record "CSV Buffer" temporary;
        Client: HttpClient;
        ...
        Row: Integer;
    begin
        if not Client.Get(GetTranslationURL, ResponseMessage) then
        ...
        Column := 2;
        repeat
            validate("Language Code", CSVBuffer.GetValue(1, Column));
    ...
    local procedure SetIsObjectTranslation()
    begin
        "Is Object Translation" := "Report ID" + "Table No." + "Field No." <> 0;
    end;

    procedure GetTranslationURL() TranslationURL: Text
    var
        Handled: Boolean;
    begin
        OnBeforeGetTranslationURL(TranslationURL, Handled);
        if Handled and (TranslationURL <> '') then
            exit(TranslationURL);
    ...
    var
        l: Record "ForNAV Language Setup";
        c: Integer;
    ...
    begin
        ...
        if ("Table No." = 0) and ("Field No." = 0) and ("Report ID" = 0) then
            l.SetRange(l."Translate From", "Translate From");
        c := l.Count();
        if (c > 0) then
            Error(lblDuplicateLcids, "Windows Language ID")
    end;
    ...
    procedure OnBeforeGetTranslationURL(var TranslationURL: Text; var Handled: Boolean)
    begin
    end;
}
