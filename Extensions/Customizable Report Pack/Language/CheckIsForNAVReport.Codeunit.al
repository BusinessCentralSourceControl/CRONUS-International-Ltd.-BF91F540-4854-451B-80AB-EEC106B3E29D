codeunit 6189181 "ForNAV Check Is ForNAV Report"
{
    ...
    procedure GetAllForNAVReports(var ReportObject: Record "ForNAV Report Object"; FromReportID: Integer; ToReportID: Integer)
    var
    ...
    begin
        ...
        if AllObjWithCaption.FindSet then
            repeat
                if IsForNAVReport(AllObjWithCaption."Object ID") then begin
    ...
    procedure GetZipContents(var TempBlob: Codeunit "Temp Blob"; Value: Text; var InStr: InStream): Boolean
    var
        OutStr: OutStream;
    ...
    begin
        ...
        if not ZipEntryList.Contains(Value) then
            exit(false);
    ...
    procedure IsForNAVReport(ID: Integer): Boolean
    begin
        exit(IsForNAVRdlcReport(ID));
    end;

    procedure IsForNAVWordReport(ID: Integer): Boolean
    var
        layoutStream: InStream;
        bt: BigText;
        TempBlob: Codeunit "Temp Blob";
    begin
        if not Report.WordLayout(ID, layoutStream) then
            exit(false);

        if not GetZipContents(tempBlob, 'docProps/custom.xml', layoutStream) then
            exit(false);
    ...
    procedure IsForNAVRdlcReport(ID: Integer): Boolean
    var
        TempBlob: Record "ForNAV Language Setup";
        InStr: InStream;
        bt: BigText;
        rdlc: Text;
    begin
        if not Report.RdlcLayout(ID, InStr) then
            exit(false);
        bt.Read(InStr);
        rdlc := Format(bt);
        if not (StrPos(rdlc, '<Description>') > 0) then
            exit(false);
    ...
    local procedure ForNAVFirstTimeSetup()
    begin
