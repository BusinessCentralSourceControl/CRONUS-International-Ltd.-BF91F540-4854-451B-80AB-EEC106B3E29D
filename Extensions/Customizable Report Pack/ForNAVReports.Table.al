Table 6188475 "ForNAV Reports"
{
    ...
    fields
    {
        ...
        field(35; "Archive Enabled"; Boolean)
        {
            ...
            begin
                if ID <> 0 then
                    archiveSetup.SetArchiveSetup(ID, "Archive Enabled")
                else
    ...
    var
        ReportMgt: Codeunit "ForNAV OnPrem Report Mgt";
    begin
        ReportMgt.LaunchDesigner(ID);
    end;

    procedure GetCategory(): Option
    var
        AllObj: Record AllObj;
    begin
        if ID < 100000 then
            exit(Category::"My Report");

        if (ID < 6188471) or (ID > 6189470) then
            exit(Category::"App Report");

        AllObj.SetRange("Object Type", AllObj."Object Type"::Report);
        AllObj.SetRange("Object ID", ID);
        AllObj.FindFirst;

        if StrPos(AllObj."Object Name", 'Templ') <> 0 then
            exit(Category::Template);

        if StrPos(AllObj."Object Name", 'Service') <> 0 then
            exit(Category::Service);

        if (StrPos(AllObj."Object Name", 'Label') <> 0) or (StrPos(AllObj."Object Name", 'Price Tag') <> 0) then
            exit(Category::Label);

        if StrPos(AllObj."Object Name", 'Example') <> 0 then
            exit(Category::Example);

        if StrPos(AllObj."Object Name", 'Warehouse') <> 0 then
            exit(Category::Warehouse);

        if StrPos(AllObj."Object Name", 'Purch') <> 0 then
            exit(Category::Purchase);

        if (StrPos(AllObj."Object Name", 'Order Confirmation') <> 0) or
          (StrPos(AllObj."Object Name", 'Draft') <> 0) or
          (StrPos(AllObj."Object Name", 'Pro Forma') <> 0) or
          (StrPos(AllObj."Object Name", 'Sales') <> 0) or
          (StrPos(AllObj."Object Name", 'Salesperson') <> 0) or
          (StrPos(AllObj."Object Name", 'Credit Memo') <> 0) or
          (StrPos(AllObj."Object Name", 'Picking List') <> 0)
        then
            exit(Category::Sales);

        if (StrPos(AllObj."Object Name", 'Inventory') <> 0) or
          (StrPos(AllObj."Object Name", 'Item') <> 0)
        then
            exit(Category::Finance);

        if (StrPos(AllObj."Object Name", 'Top 10') <> 0) or
          (StrPos(AllObj."Object Name", 'Balance') <> 0) or
          (StrPos(AllObj."Object Name", 'Reconcile') <> 0) or
          (StrPos(AllObj."Object Name", 'Payments') <> 0) or
          (StrPos(AllObj."Object Name", 'Statement') <> 0) or
          (StrPos(AllObj."Object Name", 'Reminder') <> 0) or
          (StrPos(AllObj."Object Name", 'Finance') <> 0) or
          (StrPos(AllObj."Object Name", 'Aged Accounts') <> 0)
        then
            exit(Category::Finance);
    end;
    ...
    begin
        ...
        if (StrPos(AllObj."Object Name", 'US Check') <> 0) and (NOT IsSalesTax.CheckIsSalesTax) then
            exit(false);

        if not (Category in [Category::Service, Category::Sales, Category::Purchase, Category::Template]) then
            exit(true);

        if (StrPos(AllObj."Object Name", 'VAT') <> 0) and (IsSalesTax.CheckIsSalesTax) then
            exit(false);

        if (StrPos(AllObj."Object Name", 'Tax') <> 0) and (NOT IsSalesTax.CheckIsSalesTax) then
            exit(false);
        exit(true);
    end;
        ...
        Case true of
            ...
            CustomReportLayoutActivated():
                begin
                    ...
                    if RepLaySel.IsEmpty then
                        RepLaySel.SetFilter("Company Name", '%1', '');
                    RepLaySel.FindFirst();
                    Description := RepLaySel."Report Layout Description";
                    Layout := Layout::Activated;
                end;
            HasCustomReportLayout() and (not CustomReportLayoutActivated()):
                begin
    ...
    var
        ReportLayout: Record "ForNAV Report Layout" temporary;
    begin
    ...
    local procedure OnAfterGetCurrentReportLayout(var ForNAVReports: Record "ForNAV Reports")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnDrillDownDescription(var ForNAVReports: Record "ForNAV Reports")
    begin
    end;
}
