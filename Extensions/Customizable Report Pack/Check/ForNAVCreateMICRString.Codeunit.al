Codeunit 6188778 "ForNAV Create MICR String"
{
    ...
    procedure GetMICRString(var ForNAVCheckModel: Record "ForNAV Check Model")
    var
        CheckSetup: Record "ForNAV Check Setup";
    begin
        CheckSetup.Get;
        if CheckSetup."MICR Encoding" in [CheckSetup."micr encoding"::None, CheckSetup."micr encoding"::"Defined with Per-Tenant Extension"] then
            exit;
    ...
    local procedure AddZeroesToCheckNo(Value: Text): Text
    begin
        if StrLen(Value) >= 6 then
            exit(Value);

        while StrLen(Value) < 6 do
            Value := '0' + Value;
