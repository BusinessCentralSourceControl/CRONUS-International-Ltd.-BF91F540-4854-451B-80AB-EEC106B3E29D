Table 6188471 "ForNAV Setup"
{
    ...
    begin
        if CheckIsSalesTax then
            OnBeforeDesignTemplate(SalesTemplSalesTax.ObjectId(false), Handled)
        else
    ...
    begin
        if CheckIsSalesTax then
            OnBeforeDesignTemplate(PurchaseTemplTax.ObjectId(false), Handled)
        else
    ...
    procedure DesignServiceTemplate()
    var
        ServiceTemplateVAT: Report "ForNAV Service Template";
        ServiceTemplSalesTax: Report "ForNAV Service Template Tax";
        Handled: Boolean;
    begin
        if CheckIsSalesTax then
            OnBeforeDesignTemplate(ServiceTemplSalesTax.ObjectId(false), Handled)
        else
            OnBeforeDesignTemplate(ServiceTemplateVAT.ObjectId(false), Handled);
        if Handled then
            exit;
        if CheckIsSalesTax then
            ServiceTemplSalesTax.RunModal
        else
    ...
    procedure GetListReportWatermarkLandscape(): Text
    var
        TempBlob: Record "ForNAV Core Setup";
    begin
    ...
    procedure SendLicenseEmail()
    var
        OrderLicense: Codeunit "ForNAV Order License";
    begin
        OrderLicense.SendEmail;
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeDesignTemplate(ReportID: Text; var Handled: Boolean)
    begin
    end;
}
