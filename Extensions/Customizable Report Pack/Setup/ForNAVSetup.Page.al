Page 6188471 "ForNAV Setup"
{
    ...
    actions
    {
        area(processing)
        {
            ...
            action(UpdateUserAccess)
            {
                ...
                trigger OnAction()
                var
                    InstallApp: Codeunit "ForNAV Install App";
                begin
                    InstallApp.UpdateUserAccess();
                end;
            }
            ...
            group(Delete)
            {
                ...
                action(DeleteDocWatermark)
                {
                    ...
                    begin
                        if not Confirm(AreYouSureQst, false, Rec.FieldCaption("Document Watermark")) then
                            exit;
                ...
                action(DeleteListWatermark)
                {
                    ...
                    var
                        AreYouSureQst: label 'Are you sure you want to clear %1?';
                    begin
                        if not Confirm(AreYouSureQst, false, Rec.FieldCaption("List Report Watermark")) then
                            exit;
                ...
                action(DeleteListWatermarkLandscape)
                {
                    ...
                    var
                        AreYouSureQst: label 'Are you sure you want to clear %1?';
                    begin
                        if not Confirm(AreYouSureQst, false, Rec.FieldCaption("List Report Watermark (Lands.)")) then
                            exit;
                ...
                action(DeleteLogo)
                {
                    ...
                    var
                        AreYouSureQst: label 'Are you sure you want to clear %1?';
                    begin
                        if not Confirm(AreYouSureQst, false, Rec.FieldCaption(Logo)) then
                            exit;
    ...
    begin
        Rec.InitSetup;
        ShowLicenseDetails := CurrentClientType = Clienttype::Web;
    end;
