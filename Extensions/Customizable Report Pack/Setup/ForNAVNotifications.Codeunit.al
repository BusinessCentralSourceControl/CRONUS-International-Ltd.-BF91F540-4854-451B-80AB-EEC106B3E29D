Codeunit 6188490 "ForNAV Notifications"
{
    ...
    begin
        if not ForNAVSetup.ReadPermission then
            exit;
    ...
    local procedure ShowLabelNotification(ReportID: Integer)
    var
        MyNotifications: Record "My Notifications";
        ...
        NoAndDontAskAgain: label 'No, and please don''t remind me again.';
    begin
        if ForNAVLabelSetup.Get then
            exit;

        if not MyNotifications.IsEnabled(NotificationIDs.SetupForNAVLabel) then
            exit;

        if not IsLabelReport(ReportID) then
            exit;
    ...
    local procedure IsLabelReport(ReportID: Integer): Boolean
    var
