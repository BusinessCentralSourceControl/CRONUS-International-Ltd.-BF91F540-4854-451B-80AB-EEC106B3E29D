...
{
    SingleInstance = true;
    procedure AddToBuffer(ReportID: Integer)
    begin
        ReportUsageStatisticsBuffer."Entry No." := 1;
        if ReportUsageStatisticsBuffer.FindLast then
            ReportUsageStatisticsBuffer."Entry No." += 1;
    ...
    procedure SaveBufferToDatabase()
    var
        ReportUsageStatistics: Record "ForNAV Report Usage Statistics";
    begin
        if ReportUsageStatisticsBuffer.IsEmpty then
            exit;
        if not ReportUsageStatistics.WritePermission then
            exit;
        ReportUsageStatisticsBuffer.FindSet;
        repeat
            ReportUsageStatistics := ReportUsageStatisticsBuffer;
    ...
    var
        ReportUsageStatisticsBuffer: Record "ForNAV Report Usage Statistics" temporary;
