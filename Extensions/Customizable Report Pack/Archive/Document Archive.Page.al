page 6189103 "ForNAV Document Archive"
{
    ...
    actions
    {
        area(Processing)
        {
            action(View)
            {
                ...
                begin
                    if Rec.Indent = 3 then begin
                        if (Rec."Document Type" = Rec."Document Type"::preview) or (Rec."Document Type" = Rec."Document Type"::print) then
                            fileType := 'pdf'
                        else
            ...
            action(Download)
            {
                ...
                Trigger OnAction()
                var
                    DocumentArchiveMgt: codeunit "ForNAV Document Archive Mgt.";
                    DocArchive: Record "ForNAV Document Archive";
                    documentHistory: Record "ForNAV Document History";
                begin
                    CurrPage.SetSelectionFilter(DocArchive);
                    if DocArchive.Count = 0 then
                        DocArchive.SetRange(DocArchive."Archive ID", DocArchive."Archive ID");
            ...
            action(Purge)
            {
                ...
                begin
                    ...
                    if Selected.Count() = 0 then
                        Selected.SetRange(Selected."Archive ID", Selected."Archive ID");
    ...
    begin
        case Rec.Indent of
            0:
                name := Rec."Report Name";
            1:
                name := Format(Rec.Year);
            2:
                name := Format(DMY2Date(1, Rec.Month, Rec.Year), 0, '<Month Text>');
            3:
                name := Format(Rec.Created);
        end;
    end;
}
