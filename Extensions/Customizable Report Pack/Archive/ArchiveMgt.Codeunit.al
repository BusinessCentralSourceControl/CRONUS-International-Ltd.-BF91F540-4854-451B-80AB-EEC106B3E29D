codeunit 6189104 "ForNAV Document Archive Mgt."
{
    ...
    local procedure UrlEncode(uri: Text): Text
    var
        retv: Text;
        b: Char;
        i: Integer;
    ...
    begin
        ...
        for i := 1 TO STRLEN(uri) do begin
            ...
            // Full URI encode :
            if (b in [36, 38, 43, 44, 47, 58, 59, 61, 63, 64, 32, 34, 60, 62, 35, 37, 123, 125, 124, 92, 94, 126, 91, 93, 96]) or (b >= 128)
            then begin
    ...
    local procedure Archive(var archiveCollection: List of [Integer])
    var
        archiveId: Integer;
    ...
    begin
        ...
        foreach archiveId in archiveCollection do begin
            ...
            if documentArchive.Document.HasValue then begin
                ...
                if (documentArchive."Document Type" = documentArchive."Document Type"::preview) or (documentArchive."Document Type" = documentArchive."Document Type"::print) then
                    documentArchive."Document Type" := documentArchive."Document Type"::pdf;
    ...
    local procedure Purge(var archiveCollection: List of [Integer])
    var
        archiveId: Integer;
        documentHistory: Record "ForNAV Document History";
        documentArchive: Record "ForNAV Document Archive";
    ...
    local procedure SetAction(var documentArchive: Record "ForNAV Document Archive"; action: Option Created,Viewed,Downloaded,Purged) //Enum "ForNAV Document History Type")
    begin
        case action of
            action::Purged:
                documentArchive.Purged := true;
    ...
    procedure ArchiveAction(var selected: Record "ForNAV Document Archive"; action: Option Created,Viewed,Downloaded,Purged)
    var
        archiveCollection: List of [Integer];
    ...
    begin
        if selected.FindFirst() then begin
            repeat
                case selected.Indent of
                    0, 1, 2:
                        begin
                            documentArchive.SetRange(documentArchive."Report ID", selected."Report ID");
                            if selected.Indent <> 0 then
                                documentArchive.SetRange(documentArchive.Year, selected.Year);
                            if selected.Indent = 2 then
                                documentArchive.SetRange(documentArchive.Month, selected.Month);
                            documentArchive.SetRange(documentArchive.Indent, 3);
                            if documentArchive.FindFirst() then
                                repeat
                                    if not archiveCollection.Contains(documentArchive."Archive ID") then
                                        archiveCollection.Add(documentArchive."Archive ID");
                                until documentArchive.Next <> 1;
                            documentArchive.Reset();
                        end;
                    3:
                        if not archiveCollection.Contains(selected."Archive ID") then
                            archiveCollection.Add(selected."Archive ID");
                end;
        ...
        if action = action::Purged then begin
            foreach archiveId in archiveCollection do begin
                documentArchive.Get(archiveId);
                if not documentArchive.Downloaded then
                    if Confirm(goaheadlabel, false) then
                        goahead := true
                    else
                        exit;
            end;
        ...
        case action of
            action::Purged:
                begin
                    ...
                    documentArchive.SetRange(documentArchive.Purged, false)
                end;
            action::Downloaded:
                begin
                    Archive(archiveCollection);
                    documentArchive.SetRange(documentArchive.Downloaded, false)
                end;
        end;
        foreach archiveId in archiveCollection do begin
            documentArchive.Get(archiveId);
            ...
            if not documentArchive.FindFirst() then begin
                ...
                if documentArchive.FindFirst() then
                    SetAction(documentArchive, action);
            end;
            documentArchive.SetRange(documentArchive.Indent, 3);
    ...
    procedure WriteToArchiveInternal(ReportID: Integer; ReportAction: Option SaveAsPdf,SaveAsWord,SaveAsExcel,Preview,Print,SaveAsHtml; copies: Integer; var archiveKeyWords: Text; is: InStream)
    var
        documentArchive: Record "ForNAV Document Archive";
    ...
    begin
        ...
        case ReportAction of
            ReportAction::Preview:
                documentArchive."Document Type" := documentArchive."Document Type"::preview;
            ReportAction::Print:
                documentArchive."Document Type" := documentArchive."Document Type"::print;
            ReportAction::SaveAsPdf:
                documentArchive."Document Type" := documentArchive."Document Type"::pdf;
            ReportAction::SaveAsWord:
                documentArchive."Document Type" := documentArchive."Document Type"::docx;
            ReportAction::SaveAsExcel:
                documentArchive."Document Type" := documentArchive."Document Type"::xlsx;
            ReportAction::SaveAsHtml:
                documentArchive."Document Type" := documentArchive."Document Type"::html;
        end;
        ...
        if Strlen(keyWords) > 2000 then
            documentArchive.KeyWords := CopyStr(keyWords, 1, 2000)
        else
    ...
    procedure WriteToArchive(ReportID: Integer; ReportAction: Option SaveAsPdf,SaveAsWord,SaveAsExcel,Preview,Print,SaveAsHtml; copies: Integer; var archiveKeyWords: Text; is: InStream): Boolean
    begin
    ...
    procedure SetArchiveSetup(reportId: Integer; archive: Boolean)
    var
        archiveSetup: Record "ForNAV Document Archive Setup";
        exists: Boolean;
    begin
        ...
        if archive and not exists then begin
            ...
            if not archive and exists then
                archiveSetup.Delete();
    end;

    procedure GetArchiveSetup(reportId: Integer): Boolean
    var
        documentArchiveSetup: Record "ForNAV Document Archive Setup";
    begin
        if documentArchiveSetup.Get(0) then
            exit(true)
        else
            if documentArchiveSetup.Get(ReportID) then
                exit(true)
            else
                exit(false);
    end;

    procedure GetArchiveSetup(reportId: Integer; var count: Integer): Boolean
    var
        archive: Record "ForNAV Document Archive";
    begin
        count := 0;
        if not GetArchiveSetup(reportId) then
            exit(false);
