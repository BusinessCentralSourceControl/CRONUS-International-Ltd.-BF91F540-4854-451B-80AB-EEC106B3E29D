Table 6188481 "ForNAV Label Setup"
{
    ...
    var
        Template: Report "ForNAV Label Portrait";
        Handled: Boolean;
    begin
    ...
    var
        Template: Report "ForNAV Label Landscape";
        Handled: Boolean;
    begin
        OnBeforeDesignTemplate(Template.ObjectId(false), Handled);
        if Handled then
    ...
    procedure ShowOrDisableWarning(Value: Integer)
    var
        AskWarningQst: label 'Do you want to print %1 labels?';
        TheOptions: label 'Stop,Continue,Disable Warning';
    begin
        if Value < 10 then
            exit;

        case StrMenu(TheOptions, 1, StrSubstNo(AskWarningQst, Value)) of
            0, 1:
                Error('');
            2:
                exit;
