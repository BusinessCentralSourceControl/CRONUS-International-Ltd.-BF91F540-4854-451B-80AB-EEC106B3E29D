Report 6188661 "ForNAV Sales Shipment Label"
{
    ...
    procedure SetArgs(var ArgsSet: Record "ForNAV Label Args.")
    begin
        if not ArgsSet.IsTemporary then
            Error('Coding error, contact system admin.');
        Args.Copy(ArgsSet, true);
    end;

    procedure GetArgs(var ArgsGet: Record "ForNAV Label Args.")
    begin
        Args.FindFirst;
        if not ArgsGet.IsTemporary then
            Error('Coding error, contact system admin.');
