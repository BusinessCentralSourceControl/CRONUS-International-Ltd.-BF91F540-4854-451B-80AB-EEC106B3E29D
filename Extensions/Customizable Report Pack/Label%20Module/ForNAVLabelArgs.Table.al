Table 6188670 "ForNAV Label Args."
{
    ...
    fields
    {
        ...
        field(2; Orientation; Option) { DataClassification = SystemMetadata; OptionMembers = Undefined,Portrait,Landscape; }
        field(3; "One Label per Package"; Boolean) { DataClassification = SystemMetadata; }
        field(4; "No. of Labels"; Integer) { DataClassification = SystemMetadata; }
        field(5; "Open Designer"; Boolean) { Caption = 'Design'; DataClassification = SystemMetadata; }
        field(6; Merge; Boolean) { DataClassification = SystemMetadata; }
        field(7; "Direct Print"; Boolean) { DataClassification = SystemMetadata; }
        field(8; "Printer Name"; Text[250]) { DataClassification = SystemMetadata; }
        field(9; Blob; Blob) { Caption = 'Blob', Comment = 'DO NOT TRANSLATE'; DataClassification = SystemMetadata; }
        field(10; "Print to Stream"; Boolean) { Caption = 'Print to Stream', Comment = 'DO NOT TRANSLATE'; DataClassification = SystemMetadata; }
    }

    keys { key(Key1; "Report ID") { } }

    var
        TempBlobWithState: Record "ForNAV Core Setup" temporary;

    procedure ClearMerge()
    begin
        TempBlobWithState.Reset;
        TempBlobWithState.DeleteAll;
    end;

    procedure CreateLabels(Data: Variant)
    var
        ...
        n: Integer;
    begin
        if not LabelSetup.Get then
            LabelSetup."Enable Preview" := true;
        ...
        if FindLinesRecRef(RecRef, LineRec) then begin
            if LineRec.FindSet then
                repeat
                    ...
                    for n := 1 to FindQtyField(LineRec) do
                        CreateLabelPerLine(Label, RecRef, LineRec);
                until LineRec.Next = 0;
            LineRec.Close;
        end else begin
            Label."ForNAV Label No." := 1;
            Label."ForNAV Page No." := 0;
            for n := 1 to FindQtyField(RecRef) do
                CreateLabel(Label, RecRef);
        end;

        RunTheLabel(Data, Label, LabelSetup."Enable Preview");
    end;

    local procedure RunTheLabel(var Data: Variant; var Label: Record "ForNAV Label" temporary; DemoMode: Boolean)
    var
    ...
    begin
        ...
        if "Print to Stream" then
            Blob.CreateOutstream(OutStr);

        LabelMgt.SetData(Label);
        LabelMgt.SetMergePDF(TempBlob);
        if IsForNAVLabel("Report ID") then begin
            if Merge or "Print to Stream" then
                Report.SaveAs("Report ID", '', Reportformat::Pdf, OutStr)
            else
                if "Direct Print" then
                    DirectPrint.DirectPrint("Report ID", Label)
                else
        ...
        if Merge then
            TempBlobWithState.Insert;
    end;

    local procedure FindLinesRecRef(var RecRef: RecordRef; var LineRec: RecordRef): Boolean
    var
        FldRef: FieldRef;
    ...
    begin
        case RecRef.Number of
            ...
            Database::"Sales Invoice Header", Database::"Purch. Inv. Header":
                LineRec.Open(RecRef.Number + 1);
            Database::"Sales Shipment Header", Database::"Purch. Rcpt. Header":
                LineRec.Open(RecRef.Number + 1);
            Database::"Sales Cr.Memo Header", Database::"Purch. Cr. Memo Hdr.":
                LineRec.Open(RecRef.Number + 1);
            else
                ForNAVCreateLabelData(RecRef, LineRec, UseDefaultLineFilters, PrintOnHeader);
        end;

        if PrintOnHeader or (LineRec.Number = 0) then
            exit(false);
    ...
    local procedure CreateLabel(var Label: Record "ForNAV Label"; var RecRef: RecordRef)
    var
        Fld: Record "Field";
    ...
    begin
        ...
        if Fld.FindSet then
            repeat
                FldRef := LabelRecRef.Field(Fld."No.");
                case Fld.Type of
                    Fld.Type::Text, Fld.Type::Code:
                        FldRef.Value := FindField(RecRef, FldRef.Name, '');
                    Fld.Type::BigInteger, Fld.Type::Integer, Fld.Type::Decimal:
    ...
    local procedure CreateLabelPerLine(var Label: Record "ForNAV Label"; var RecRef: RecordRef; var LineRec: RecordRef)
    var
        Fld: Record "Field";
        LabelRecRef: RecordRef;
        FldRef: FieldRef;
    ...
    local procedure FindField(var RecRef: RecordRef; Which: Text; Value: Variant): Text
    var
        Fld: Record "Field";
        FldRef: FieldRef;
    begin
    ...
    local procedure FindFieldDec(var RecRef: RecordRef; Which: Text; Value: Variant): Decimal
    var
        Fld: Record "Field";
        FldRef: FieldRef;
    begin
    ...
    local procedure FindFieldDate(var RecRef: RecordRef; Which: Text; Value: Variant): Date
    var
        Fld: Record "Field";
        FldRef: FieldRef;
    begin
    ...
    local procedure GetBarCode(var Label: Record "ForNAV Label"; var RecRef: RecordRef; var LineRec: RecordRef)
    var
        Handled: Boolean;
    begin
    ...
    local procedure GetBarCodeEvent(var Label: Record "ForNAV Label"; var RecRef: RecordRef; var LineRec: RecordRef; var Handled: Boolean)
    begin
    end;

    local procedure GetArea(var Label: Record "ForNAV Label"; var RecRef: RecordRef; var LineRec: RecordRef)
    var
        Handled: Boolean;
    begin
    ...
    local procedure GetAreaEvent(var Label: Record "ForNAV Label"; var RecRef: RecordRef; var LineRec: RecordRef; var Handled: Boolean)
    begin
    end;

    local procedure GetLocation(var Label: Record "ForNAV Label"; var RecRef: RecordRef)
    var
    ...
    begin
        ...
        if not Location.Get(FldRef.Value) then
            exit;
    ...
    local procedure UseDocumentType(var RecRef: RecordRef): Boolean
    begin
        exit(RecRef.Number in [Database::"Sales Header", Database::"Purchase Header"]);
    end;

    [IntegrationEvent(false, false)]
    local procedure ForNAVRunCustomLabelReport(var Data: Variant; var Label: Record "ForNAV Label" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure ForNAVCreateLabelData(var RecRef: RecordRef; var LineRec: RecordRef; var UseDefaultLineFilters: Boolean; var PrintOnHeader: Boolean)
    begin
    end;

    local procedure FindQtyField(var RecRef: RecordRef): Integer
    var
        Fld: Record "Field";
        FldRef: FieldRef;
        Value: Decimal;
    begin
        if not "One Label per Package" then
            exit(1);
        ...
        FldRef := RecRef.Field(Fld."No.");
        case Fld.Type of
            Fld.Type::Integer, Fld.Type::BigInteger:
                exit(FldRef.Value);
            Fld.Type::Decimal:
                begin
                    Value := FldRef.Value;
    ...
    procedure GetOrientationForRequestPage()
    var
        LabelSetup: Record "ForNAV Label Setup";
    begin
        if not LabelSetup.Get then
        ...
        case LabelSetup.Orientation of
            LabelSetup.Orientation::Landscape:
                Orientation := Orientation::Landscape;
            LabelSetup.Orientation::Portrait:
                Orientation := Orientation::Portrait;
        end;
    end;

    local procedure IsForNAVLabel(Value: Integer) ReturnValue: Boolean
    var
        Handled: Boolean;
    begin
    ...
    local procedure SetIsForNAVLabel(Value: Integer; var ReturnValue: Boolean)
    begin
    end;
}
